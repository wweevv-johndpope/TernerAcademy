...
(
    SELECT T1.Id as CourseId, 
           ...
           T1.RatingCount as CourseRatingCount,
    ...
    GROUP BY T1.Id, T1.Name, T1.ShortDescription, T1.ThumbnailImageUri, T1.[Level], T1.PriceInTFuel, T1.ListingStatus,  T1.AverageRating, T1.RatingCount, T1.EnrolledCount, T1.CreatedAt,  T2.Id, T2.Name, T3.Id, T3.Name, T3.ProfilePictureFilename
) AS TT1 
