...
namespace Application.InstructorPortal.Account.Commands.UploadProfilePicture
{
    public class UploadProfilePictureCommand : IRequest<IResult>
    {
        ...
        public class UploadProfilePictureCommandHandler : IRequestHandler<UploadProfilePictureCommand, IResult>
        {
            ...
            public UploadProfilePictureCommandHandler(ICallContext context, IInstructorIdentityService identityService, IAzureStorageBlobService blobService)
            {
                _context = context;
            ...
            public async Task<IResult> Handle(UploadProfilePictureCommand request, CancellationToken cancellationToken)
            {
                ...
                string filename = $"Instructor-{Guid.NewGuid()}{request.FileExtension}".ToLower();
