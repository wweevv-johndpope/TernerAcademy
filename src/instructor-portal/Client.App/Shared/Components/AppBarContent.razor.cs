...
using Client.App.Pages.Wallets.Modal;
...
namespace Client.App.Shared.Components
{
    public partial class AppBarContent : IDisposable
    {
        ...
        private async Task InvokeUpdateWalletModalAsync()
        {
            var parameters = new DialogParameters
            {
                { nameof(UpdateWalletModal.Model), new UpdateWalletCommand() { WalletAddress = WalletAddress } }
            };

            var dialog = _dialogService.Show<UpdateWalletModal>("Update Wallet", parameters);
